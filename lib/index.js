...
exports.createRouter = function createRouter(options, callback) {
  ...
  rewrites = {
    list: options.rewrites.filter(function (rr) {
      return /^_list/.test(rr.to) && !/.byUser./.test(rr.to)
        || /^\/-\/_view/.test(rr.from)
        || /^\/-\/_show/.test(rr.from);
    }),
    usersList: options.rewrites.filter(function (rr) {
      return /^_list/.test(rr.to) && /.byUser./.test(rr.to);
    }),
  ...
  // Remark: This now defaults to private
  //
  rewrites.list.forEach(function (rr) {
    ...
    log.info('[route]', '%s - %s %s', 'private', method, rr.from);
  ...
  //
  // These are list functions specifically relating to
  // users that will stay public
  //
  rewrites.usersList.forEach(function (rr) {
    var wc     = patterns.wildcard,
        method = rr.method.toLowerCase(),
        from   = wc.test(rr.from)
          ? new RegExp(rr.from.replace(wc, '.*'))
